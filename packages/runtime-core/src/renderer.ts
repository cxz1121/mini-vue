...
export function createRenderer(renderOptions) {
  ...
  const patchChildren = (n1, n2, el) => {
    ...
    if (newShapeFlag && newShapeFlag & ShapeFlags.TEXT_CHILDREN) {
      //新的是文本
      ...
      if (preShapeFlag && preShapeFlag & ShapeFlags.ARRAY_CHILDREN) {
        //老的是数组
        if (newShapeFlag && newShapeFlag & ShapeFlags.ARRAY_CHILDREN) {
          //老的是数组，新的是数组
          //diff 8
        ...
        //老的是文本 或 空，新的是数组 或 空 2 3 5 6
        if (preShapeFlag && preShapeFlag & ShapeFlags.TEXT_CHILDREN) {
          //老的是文本 设置为 空，包含了老的为空
